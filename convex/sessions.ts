...
// Get the number of students in a session
...
// Get the number of currently lost students
...
export const setLostStatus = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (studentRecord) {
      ...
      // Logic for if student record is missing (should verify robustly in prod)
      // For now, we assume if they are calling this, they joined.
      // But if we cleared DB or they are from old session without record:
       await ctx.db.insert("students", {
    ...
    // Also trigger the event for analytics/teacher view spike detection
    if (args.isLost) {
